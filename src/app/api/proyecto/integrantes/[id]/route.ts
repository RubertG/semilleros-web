...
export async function GET(request: Request, context: Context) {
  ...
  const { data: proyecto } = await supabase.from("Estudiante_proyecto").select("proyecto:id_proyecto(id,nombre),estudiante:id_estudiante(id,nombre)").eq("id_proyecto", id);
